...
func (r *WithdrawalRequestRepo) CountByStatus(ctx context.Context, statuses ...string) (map[string]int64, error) {
...
func (r *WithdrawalRequestRepo) CreateRecoverWithHold(ctx context.Context, request *models.WithdrawalRequest, ledger *LedgerRepo) error {
	if ledger == nil {
		return ErrLedgerReservationRequired
	}
	if request.ID == uuid.Nil {
...
func (r *WithdrawalRequestRepo) ListPage(ctx context.Context, page, limit int, status string) ([]models.WithdrawalRequest, int64, error) {
	if page < 1 {
		page = 1
	}
	if limit < 1 || limit > 200 {
	...
	var total int64
	countQuery := r.db.WithContext(ctx).Model(&models.WithdrawalRequest{})
	if status != "" {
	...
	var requests []models.WithdrawalRequest
	query := r.db.WithContext(ctx).Preload("Merchant").Preload("Wallet")
	if status != "" {
		query = query.Where("status = ?", status)
	}
	err := query.Order("created_at DESC").Limit(limit).Offset((page - 1) * limit).Find(&requests).Error
	return requests, total, err
}
...
func (r *WithdrawalRequestRepo) SumActiveAmountRawByMerchantSince(ctx context.Context, merchantID uuid.UUID, chain string, token *string, symbol string, since time.Time) (*big.Int, error) {
...
func (r *WithdrawalRequestRepo) ApproveWithTransfer(ctx context.Context, id uuid.UUID, reviewedBy string, ledger *LedgerRepo, transfer func(*models.WithdrawalRequest) (string, error)) (*models.WithdrawalRequest, error) {
	...
	if transferErr != nil {
		if OutboundTransferFailureBroadcastUncertain(transferErr) {
			...
			request.Status = models.WithdrawalStatusProcessing
			if persistErr := r.SetProcessingError(ctx, id, errText); persistErr != nil {
				log.Printf("withdrawal processing error persist request_id=%s error=%v", id, persistErr)
				return &request, errors.Join(transferErr, fmt.Errorf("persist uncertain broadcast state: %w", persistErr))
			}
			request.Error = errText
			return &request, transferErr
		}
		if persistErr := r.MarkFailed(ctx, id, reviewedBy, transferErr.Error()); persistErr != nil {
			log.Printf("withdrawal failure persist request_id=%s error=%v", id, persistErr)
			request.Status = models.WithdrawalStatusProcessing
			return &request, errors.Join(transferErr, fmt.Errorf("persist withdrawal failure: %w", persistErr))
		}
	...
	if err := r.RecordBroadcast(ctx, id, reviewedBy, txHash); err != nil {
		...
		if persistErr := r.SetProcessingError(ctx, id, errText); persistErr != nil {
			log.Printf("withdrawal broadcast persistence recovery request_id=%s error=%v", id, persistErr)
			return &request, errors.Join(err, fmt.Errorf("persist broadcast recovery state: %w", persistErr))
		}
		request.Error = errText
		return &request, err
	}
...
func (r *WithdrawalRequestRepo) ApproveForOutbound(ctx context.Context, id uuid.UUID, reviewedBy string, ledger *LedgerRepo, outbound *OutboundTransactionRepo) (*models.WithdrawalRequest, *models.OutboundTransaction, error) {
	if ledger == nil {
		return nil, nil, ErrLedgerReservationRequired
	}
	if outbound == nil {
		return nil, nil, ErrOutboundTransactionInvalid
	}

	var request models.WithdrawalRequest
	var outboundTx *models.OutboundTransaction
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
			Preload("Merchant").
			Preload("Wallet").
		...
		created, _, err := NewOutboundTransactionRepo(tx).CreateForWithdrawal(ctx, request, reviewedBy)
		if err != nil {
			return err
		}
		outboundTx = created
