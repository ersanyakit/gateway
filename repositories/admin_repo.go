...
func (r *AdminRepo) SetRole(ctx context.Context, id uuid.UUID, role string) error {
	result := r.db.WithContext(ctx).Model(&models.Admin{}).
		Where("id = ?", id).
		Updates(map[string]any{"role": models.NormalizeAdminRole(role), "updated_at": time.Now()})
	if result.Error != nil {
		return result.Error
	}
...
func (r *AdminRepo) CountActivePrivileged(ctx context.Context) (int64, error) {
	var count int64
