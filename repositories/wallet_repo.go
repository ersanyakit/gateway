...
type WalletRepo struct {
	...
	addressObserver func(models.Wallet)
}

func (r *WalletRepo) DB() *gorm.DB {
	if r == nil || r.domainRepo == nil {
		return nil
	}
...
// SetAddressObserver installs the synchronous ownership-index hook used by
// chain listeners. The observer is notified only after address persistence
// succeeds.
func (r *WalletRepo) SetAddressObserver(observer func(models.Wallet)) {
	if r == nil {
		return
	}
	r.observerMu.Lock()
	r.addressObserver = observer
	r.observerMu.Unlock()
}

func (r *WalletRepo) notifyAddressObserver(wallet models.Wallet) {
	if r == nil {
		return
	}
	r.observerMu.RLock()
	observer := r.addressObserver
	r.observerMu.RUnlock()
	if observer == nil {
		return
	}
	_ = helpers.RunSafely("wallet-address-observer", func() {
		observer(wallet)
	})
}
...
func (r *WalletRepo) getNextHDIndex(ctx context.Context, db *gorm.DB, merchantID, domainID uuid.UUID) (uint32, error) {
	next, err := NewWalletAddressRepo(db).PeekNextHDIndex(ctx, db, merchantID, domainID)
	if isMissingWalletAddressTableError(err) {
		return r.getNextLegacyHDIndex(ctx, db, merchantID, domainID)
	}
	return next, err
}

func (r *WalletRepo) getNextLegacyHDIndex(ctx context.Context, db *gorm.DB, merchantID, domainID uuid.UUID) (uint32, error) {
	var maxIndex uint32
...
func (r *WalletRepo) lockLegacyHDIndex(ctx context.Context, tx *gorm.DB, merchantID, domainID uuid.UUID, hdAccountID uint32) error {
	if tx.Dialector == nil || tx.Dialector.Name() != "postgres" {
		return nil
	}
	key := fmt.Sprintf("wallet-legacy-hd-index:%s:%s:%d", merchantID, domainID, hdAccountID)
...
func (r *WalletRepo) Create(params types.WalletParams) (wallet *models.Wallet, err error) {
	if params.Context == nil {
		return nil, repositoryOperationError("wallet create", "context is required")
	}
	if r == nil || r.DB() == nil {
		return nil, repositoryOperationError("wallet create", "database is not configured")
	}
	if r.domainRepo.MerchantRepo() == nil || r.domainRepo.MerchantRepo().Blockchains() == nil {
		return nil, repositoryOperationError("wallet create", "blockchain factory is not configured")
	}
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			wallet = nil
			err = recoverRepositoryTransactionPanic("wallet create", tx, recovered)
		}
	}()
	...
	if err != nil {
		tx.Rollback()
		log.Printf("wallet create domain lookup failed: %v\n", err)
		return nil, err
	}
	...
	if err == nil {
		...
		existingWallet, err := r.FindByID(params.Context, existing.ID)
		if err != nil {
			return nil, err
		}
		if err := r.upsertAddressLookup(params.Context, *existingWallet); err != nil {
			return nil, err
		}
		if err := r.upsertWalletAddressPool(params.Context, *existingWallet); err != nil {
			return nil, err
		}
		r.notifyAddressObserver(*existingWallet)
	...
	poolReady := true
	reservation, err := addressRepo.ReserveNextHDIndexTx(params.Context, tx, WalletAddressReservationRequest{
		MerchantID:  merchantUUID,
		DomainID:    domainUUID,
		ProductID:   *params.ProductId,
		UserID:      *params.UserId,
		HDAccountID: domain.HDAccountID,
		Purpose:     WalletAddressPurposeForProduct(*params.ProductId, *params.UserId),
	})
	if err != nil {
		if !isMissingWalletAddressTableError(err) {
			tx.Rollback()
		...
		poolReady = false
		if err := r.lockLegacyHDIndex(params.Context, tx, merchantUUID, domainUUID, domain.HDAccountID); err != nil {
			tx.Rollback()
			return nil, err
		}
	}
	var hdAccountId uint32
	if poolReady {
		hdAccountId = reservation.HDAddressID
	} else {
		hdAccountId, err = r.getNextLegacyHDIndex(params.Context, tx, merchantUUID, domainUUID)
		if err != nil {
			tx.Rollback()
			return nil, err
		}
	...
	if poolReady {
		if err := addressRepo.AttachWalletToReservationTx(params.Context, tx, reservation.ID, wallet.ID); err != nil {
			tx.Rollback()
			return nil, err
		}
		if err := addressRepo.UpsertWalletTx(params.Context, tx, *wallet); err != nil {
			tx.Rollback()
			return nil, err
		}
	}
	...
	created, err := r.FindByID(params.Context, wallet.ID)
	if err != nil {
		return nil, err
	}
	if err := r.upsertAddressLookup(params.Context, *created); err != nil {
		return nil, err
	}
	if err := r.upsertWalletAddressPool(params.Context, *created); err != nil {
		return nil, err
	}
	r.notifyAddressObserver(*created)
	return created, nil
}
...
func (r *WalletRepo) CreateReserveWallet(ctx context.Context, merchantID, domainID uuid.UUID, hdAccountID uint32) (*models.Wallet, error) {
	...
	if err := r.DB().WithContext(ctx).
		...
		wallet, err := r.FindByID(ctx, existing.ID)
		if err != nil {
			return nil, err
		}
		if err := r.upsertWalletAddressPool(ctx, *wallet); err != nil {
			return nil, err
		}
		r.notifyAddressObserver(*wallet)
		return wallet, nil
	}
	...
	if err := r.upsertAddressLookup(ctx, *wallet); err != nil {
		return nil, err
	}
	if err := r.upsertWalletAddressPool(ctx, *wallet); err != nil {
		return nil, err
...
func (r *WalletRepo) ListByIDs(ctx context.Context, ids []uuid.UUID) ([]models.Wallet, error) {
	if len(ids) == 0 {
	...
	byID := make(map[uuid.UUID]models.Wallet, len(wallets))
	for _, wallet := range wallets {
		byID[wallet.ID] = wallet
	}
	ordered := make([]models.Wallet, 0, len(wallets))
	for _, id := range orderedIDs {
		if wallet, ok := byID[id]; ok {
...
func (r *WalletRepo) FindByChainAddress(ctx context.Context, chainID constants.ChainID, address string) (*models.Wallet, error) {
	...
	addressRepo := NewWalletAddressRepo(r.DB())
	wallet, err := addressRepo.FindWallet(ctx, chainID, address)
	if err == nil {
		return wallet, nil
	}
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		terminal, terminalErr := addressRepo.HasTerminalAddress(ctx, chainID, address)
		if terminalErr != nil && !isMissingWalletAddressTableError(terminalErr) {
	...
	lookupRepo := NewWalletAddressLookupRepo(r.DB())
	wallet, err = lookupRepo.FindWallet(ctx, chainID, address)
	if err == nil {
		return wallet, nil
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) && !isMissingWalletAddressLookupTableError(err) {
		return nil, err
	}

	db := r.DB().WithContext(ctx).Preload("Domain")
	var legacyWallet models.Wallet

	switch chainID {
	case constants.Bitcoin:
		err = db.First(&legacyWallet, "bitcoin_address = ?", address).Error
	case constants.Ethereum:
		err = db.First(&legacyWallet, "LOWER(ethereum_address) = LOWER(?)", address).Error
	case constants.Avalanche:
		err = db.First(&legacyWallet, "LOWER(avalanche_address) = LOWER(?)", address).Error
	case constants.Binance:
		err = db.First(&legacyWallet, "LOWER(binance_address) = LOWER(?)", address).Error
	case constants.Base:
		err = db.First(&legacyWallet, "LOWER(base_address) = LOWER(?)", address).Error
	case constants.Arbitrum:
		err = db.First(&legacyWallet, "LOWER(arbitrum_address) = LOWER(?)", address).Error
	case constants.Unichain:
		err = db.First(&legacyWallet, "LOWER(unichain_address) = LOWER(?)", address).Error
	case constants.TRON, constants.TRONTestnet:
		err = db.First(&legacyWallet, "tron_address = ?", address).Error
	case constants.Solana:
		err = db.First(&legacyWallet, "solana_address = ?", address).Error
	case constants.Chiliz:
		err = db.First(&legacyWallet, "LOWER(chiliz_address) = LOWER(?)", address).Error
	case constants.ChilizSpicy:
		err = db.First(&legacyWallet, "LOWER(chiliz_spicy_address) = LOWER(?)", address).Error
	default:
	...
	if err := r.upsertAddressLookup(ctx, legacyWallet); err != nil && !isMissingWalletAddressLookupTableError(err) {
		return nil, err
	}
	if err := r.upsertWalletAddressPool(ctx, legacyWallet); err != nil && !isMissingWalletAddressTableError(err) {
		return nil, err
	}
...
func chainToFieldName(chainName string) (string, error) {
	...
	case "tron", "tron-testnet", "trx-testnet", "nile", "tron-nile", "trx-nile", "shasta", "tron-shasta":
...
func (r *WalletRepo) FillChainAddress(ctx context.Context, walletID uuid.UUID, chainName string, blockchains *blockchain.ChainFactory) (*models.Wallet, error) {
	...
	updated, err := r.FindByID(ctx, walletID)
	if err != nil {
		return nil, err
	}
	if err := r.upsertAddressLookup(ctx, *updated); err != nil {
		return nil, err
	}
	if err := r.upsertWalletAddressPool(ctx, *updated); err != nil {
		return nil, err
	}
	r.notifyAddressObserver(*updated)
	return updated, nil
}
...
func (r *WalletRepo) EnsureAllAddresses(ctx context.Context, walletID uuid.UUID, blockchains *blockchain.ChainFactory) error {
	...
	if err := r.upsertAddressLookup(ctx, *updated); err != nil && !isMissingWalletAddressLookupTableError(err) {
		return err
	}
	if err := r.upsertWalletAddressPool(ctx, *updated); err != nil && !isMissingWalletAddressTableError(err) {
		return err
	}
	r.notifyAddressObserver(*updated)
	return nil
...
func (r *WalletRepo) upsertAddressLookup(ctx context.Context, wallet models.Wallet) error {
	err := NewWalletAddressLookupRepo(r.DB()).UpsertWallet(ctx, wallet)
	if isMissingWalletAddressLookupTableError(err) {
		return nil
	}
	return err
}

func (r *WalletRepo) upsertWalletAddressPool(ctx context.Context, wallet models.Wallet) error {
	err := NewWalletAddressRepo(r.DB()).UpsertWallet(ctx, wallet)
	if isMissingWalletAddressTableError(err) {
		return nil
	}
	return err
...
func isMissingWalletAddressLookupTableError(err error) bool {
	if err == nil {
		return false
	}
	msg := strings.ToLower(err.Error())
	return strings.Contains(msg, "wallet_address_lookups") &&
		(strings.Contains(msg, "does not exist") || strings.Contains(msg, "no such table"))
}

func isMissingWalletAddressTableError(err error) bool {
	if err == nil {
		return false
	}
	msg := strings.ToLower(err.Error())
	if !(strings.Contains(msg, "wallet_addresses") ||
		strings.Contains(msg, "wallet_address_reservations") ||
		strings.Contains(msg, "wallet_address_gap_scan")) {
		return false
	}
