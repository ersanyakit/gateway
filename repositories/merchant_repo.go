...
func (r *MerchantRepo) Create(params types.MerchantParams) (merchant *models.Merchant, err error) {
	if params.Context == nil {
		return nil, repositoryOperationError("merchant create", "context is required")
	}
	if r == nil || r.DB() == nil {
		return nil, repositoryOperationError("merchant create", "database is not configured")
	}
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			merchant = nil
			err = recoverRepositoryTransactionPanic("merchant create", tx, recovered)
		}
	}()
...
func (r *MerchantRepo) FindAnyByID(ctx context.Context, id uuid.UUID) (*models.Merchant, error) {
	var merchant models.Merchant
...
func (r *MerchantRepo) CreateDomain() (*models.Domain, error) {
	...
	merchantID, err := uuid.NewV7()
	if err != nil {
		log.Printf("merchant create domain merchant UUID generation failed: %v", err)
		return nil, fmt.Errorf("generate merchant UUID: %w", err)
	}
	...
	if err != nil {
		log.Printf("merchant create domain UUID generation failed: %v", err)
		return nil, fmt.Errorf("generate domain UUID: %w", err)
	}
