...
type PaymentCreateParams struct {
	...
	RequiredMemo     *string `json:"required_memo,omitempty"`
...
func (p *PaymentCreateParams) Validate() error {
	...
	p.RequiredMemo = trim(p.RequiredMemo)
	if p.SettlementPolicy != nil {
		policy := strings.ToLower(strings.TrimSpace(*p.SettlementPolicy))
		switch policy {
		case "", "single":
			policy = "single"
		case "aggregate":
		default:
			return errors.New("SettlementPolicy must be single or aggregate")
		}
		p.SettlementPolicy = &policy
	}
	return nil
}
