...
1. Given an admin or dealer portal action can affect outbound money movement, when the action is submitted, then CSRF/session protections, actor authorization, and tenant/domain scope checks run before state changes, and failures do not leak sensitive resource existence.
...
4. Given security hardening is implemented, when automated tests run, then they cover CSRF enforcement, role separation, scope isolation, whitelist rejection, velocity limit rejection, emergency freeze, audit log creation, and sensitive data redaction.

## Tasks / Subtasks

- [ ] Task 1: Harden portal CSRF/session and authorization gates before mutation (AC: 1, 4)
  - [ ] Verify `/dealer`, `/merchant`, and `/admin` POST routes remain behind `middleware.PortalCSRF()` in `api/routes/routes.go`; add route/source-contract coverage so future route additions cannot bypass it.
  - [ ] Ensure server-rendered admin/dealer forms that POST include a valid `_csrf` field or a supported `X-CSRF-Token` path; do not rely on cookies alone for unsafe methods.
  - [ ] Add focused middleware/route tests for safe method token issuance, unsafe POST rejection without token, session-bound token mismatch rejection, and successful POST with valid token.
  - [ ] Keep `RequireAdmin` active-account verification as the authoritative admin session boundary; do not trust `requireAdmin` unless `RequireAdmin` has already populated `admin_session_email`.
  - [ ] Add or extend an admin role/permission guard for high-risk actions. At minimum, approval/recover/security-policy mutations must reject non-privileged admins before resource lookup or state mutation.
  - [ ] Preserve generic failure messages for unauthorized, wrong-scope, missing-resource, and policy-denied paths. Do not reveal whether another tenant/domain owns a wallet, payout, refund, webhook delivery, or policy record.

- [ ] Task 2: Add outbound security policy storage and enforcement boundary (AC: 2, 4)
  - [ ] Add narrowly scoped models/repositories for outbound policy state instead of scattering env-only checks through handlers. Suggested tables: outbound policy settings and outbound address whitelist entries.
  - [ ] Policy settings must support default-off behavior for address whitelist requirement, per-tenant amount limit, rolling velocity window limit, and emergency freeze. Emergency freeze must block signing/broadcast for withdrawal, payout/recover, refund, and sweep-relevant admin outbound paths.
  - [ ] Whitelist entries must be scoped by merchant/domain where applicable plus chain/token/address, with normalized address comparison suitable for the chain family. Do not require a live chain call for whitelist validation.
  - [ ] Velocity and per-tenant limits must use ledger/lifecycle persisted data, not live chain balances or transaction-row sums. Use raw integer strings and existing chain/token metadata.
  - [ ] Enforce policy before any signing/broadcast callback. For pending request creation, fail before `CreateWithHold` where possible; for approval/recover/refund paths, fail before `ExecuteReservedWalletTransfer`.
  - [ ] Policy denials must leave existing holds safe: pre-broadcast denials can fail/reject and void according to the current repository semantics; post-broadcast states must not be blindly voided or retried.
  - [ ] Add repository tests for policy lookup precedence, whitelist match/miss, freeze blocking, limit math, and duplicate-safe policy/whitelist persistence.

- [ ] Task 3: Extend append-only audit evidence for high-risk decisions (AC: 2, 3, 4)
  - [ ] Extend `models.ActivityLog` and `repositories.ActivityLogRepo` only as needed to capture structured scope and decision evidence: domain id, actor role, decision, reason, before status, after status, and correlation id.
  - [ ] Make activity logs append-only by design. Prefer model hooks or repository constraints that reject update/delete attempts, plus tests proving audit records cannot be mutated or deleted through the repository/ORM path.
  - [ ] Replace free-form-only audit calls for high-risk paths with a helper that records before/after status and policy decision without leaking secrets or raw signatures.
  - [ ] Cover these events at minimum: dealer withdrawal create, admin withdrawal approve/reject, admin refund approve/reject, admin recover funds, domain API secret rotation, webhook replay, admin account/TOTP changes, emergency freeze changes, whitelist changes, and policy limit changes.
  - [ ] Ensure audit records include request path, method, IP, user agent, and `middleware.RequestIDFromCtx(c)` correlation id as `logDealerActivity` currently does.
  - [ ] Add redaction tests so API secrets, webhook secrets, raw signatures, mnemonics, private keys, raw signed transactions, and unbounded diagnostic blobs are never written to audit descriptions or structured metadata.

- [ ] Task 4: Surface operator controls without weakening UX or security (AC: 1, 2, 3)
  - [ ] Add admin security/policy controls to the existing server-rendered admin dashboard; keep the work-focused table/detail-panel style from `views/dealer/admin_dashboard.html`.
  - [ ] Show emergency freeze state, whitelist entries, policy limits, actor, scope, timestamp, and last decision near the controls. Do not hide audit context behind hover-only UI.
  - [ ] Add confirmation affordances for destructive/high-risk policy toggles; use existing button/input styles and status colors, not accent color as success/warning/danger.
  - [ ] Do not display secrets after rotation. Domain API secret rotation may return the new secret once as it does today; audit output and activity tables must stay redacted.
  - [ ] Update CSS/JS only if needed for the admin security controls; avoid SPA framework or decorative redesign.

- [ ] Task 5: Keep schemas, docs, and production-readiness evidence synchronized (AC: 1, 2, 3, 4)
  - [ ] Register any new model in `services/database.autoMigrateModels`.
  - [ ] Add required columns/indexes to `services/database.VerifySchema` and matching schema tests.
  - [ ] Update `docs/integration-guide.md`, Swagger docs if public/admin behavior is documented there, and readiness/audit docs if the launch posture changes.
  - [ ] Add readiness checks for production policy posture if implemented as required launch gates: CSRF secret, emergency freeze visibility, privileged admin role coverage, and outbound policy configuration.

- [ ] Task 6: Add focused validation and update story records (AC: 1, 2, 3, 4)
  - [ ] Add handler/source-contract tests for CSRF coverage, role separation, no resource existence leakage, policy guard placement before signing, and scoped audit calls.
  - [ ] Add repository tests for policy, whitelist, velocity/tenant limits, audit immutability, and schema registration.
  - [ ] Add view/template tests for admin security controls and audit metadata visibility.
  - [ ] Run targeted validation: `GOCACHE=/tmp/gateway-gocache-bmad go test -p=1 -count=1 ./api/middleware ./api/handlers ./repositories ./services/database`.
  ...
  - [ ] Update Dev Agent Record, Completion Notes, File List, Change Log, and move the story only to `review` after validation passes.

## Dev Notes

### Current Implementation Snapshot

- Portal CSRF already exists in `api/middleware/csrf_jwt.go` and is applied to `/dealer`, `/merchant`, and `/admin` in `api/routes/routes.go`. It issues JWT-style CSRF tokens on safe methods and verifies `_csrf` form field or `X-CSRF-Token` header on unsafe methods. Current test coverage is readiness-oriented; add actual middleware/route behavior tests.
...
- FR39 requires CSRF audit, role separation, key rotation policy, IP/device/session controls, immutable audit trail, address whitelist, velocity limits, dual approval, and emergency freeze behavior.
...
- CSRF tests should exercise the real `PortalCSRF` middleware behavior, not only readiness env checks.
...
- Handler/source-contract tests may continue using existing string-contract patterns where useful, but money/security behavior also needs executable tests for policy and CSRF.
...
- Do not introduce an authorization framework or CSRF library unless existing middleware is proven insufficient. The current local middleware is small, tested, and already wired into portal routes.
...
- 2026-06-28: Story created from Epic 4.6 after Story 4.5 reached done. Existing CSRF middleware, admin session/TOTP model, activity log model/repo, admin routes, outbound approval/recover/refund handlers, database schema verification, UX audit guidance, and previous Epic 4 story records inspected.

### Completion Notes List

- Ultimate context engine analysis completed - comprehensive developer guide created.
- Story is ready for development; no code implementation has been performed for Story 4.6 yet.
...
- 2026-06-28: Created ready-for-dev Story 4.6 with admin CSRF/session hardening, role/permission guardrails, outbound policy/whitelist/limit/freeze scope, append-only audit requirements, UI/docs/schema guidance, and validation plan.
