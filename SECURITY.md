...
- `SIGNER_MODE=kms`, `hsm`, `mpc`, `vault`, or another external custody mode requires an active custody adapter. Without one, production signing fails with an explicit integration-required error.
- Production wallet address creation uses the custody adapter as a watch-only derivation boundary. Returned wallet metadata must contain address/key reference only; mnemonic/private key fields are JSON-omitted and remain empty.
- The current chain transaction builders still use local private-key signing in non-production. In production those local secret-material boundaries fail closed even if an external adapter is registered, until chain-specific adapter signing is implemented.
...
- `/metrics` exposes Prometheus-compatible operational gauges for backlog, reconciliation, chain state, migration policy, signer policy, and external signer adapter readiness.
