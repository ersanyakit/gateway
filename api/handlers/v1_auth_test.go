...
func TestV1ResolveSignedDomainEnforcesScopeBeforeBusinessLogic(t *testing.T) {
	v1SignedReplayGuard = newV1SignatureReplayGuard(2 * time.Minute)

	domain := &models.Domain{ID: uuid.New(), APIKey: "key-1", APIScopes: models.DomainAPIScopeRead}
	lookup := fakeV1DomainLookup{
		byKey:    map[string]*models.Domain{"key-1": domain},
		bySecret: map[string]*models.Domain{"secret-1": domain},
	}
	...
	app := fiber.New()
	app.Post("/probe", func(c fiber.Ctx) error {
		_, err := v1ResolveSignedDomainForScope(c, lookup, models.DomainAPIScopePaymentCreate)
		if err != nil {
			return v1Err(c, fiber.StatusUnauthorized, err.Error())
		}
		called = true
	...
	if called {
		t.Fatal("business logic ran despite missing scope")
	}
	envelope := readV1ErrorEnvelope(t, resp)
	if envelope["message"] != "API key scope is not allowed" {
		t.Fatalf("envelope = %#v", envelope)
	}
}

func TestV1ResolveDomainEnforcesIPAllowlist(t *testing.T) {
	domain := &models.Domain{
		ID:             uuid.New(),
		APIKey:         "key-1",
		APIScopes:      models.DefaultDomainAPIScopesCSV(),
		APIIPAllowlist: "203.0.113.10",
	}
	lookup := fakeV1DomainLookup{byKey: map[string]*models.Domain{"key-1": domain}}
	called := false
	...
	app := fiber.New()
	app.Get("/probe", func(c fiber.Ctx) error {
		_, err := v1ResolveDomain(c, lookup)
		if err != nil {
			return v1Err(c, fiber.StatusUnauthorized, err.Error())
		}
		called = true
	...
	if called {
		t.Fatal("business logic ran despite IP allowlist denial")
	}
}
