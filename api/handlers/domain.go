...
// @Description Creates a merchant domain with either webhook or NATS notification settings and returns the API key used by payment requests.
...
func HandleDomainCreate(s *services.DomainService) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		if models.NormalizeDomainNotificationMode(params.NotificationMode) == models.DomainNotificationWebhook {
		...
		} else if params.NATSURL != nil {
			if err := helpers.ValidateNATSURL(*params.NATSURL); err != nil {
				return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
					"success": false,
					"error":   "Invalid NATS URL: " + err.Error(),
				})
			}
		}
