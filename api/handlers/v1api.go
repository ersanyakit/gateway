...
func v1ResolveDomain(c fiber.Ctx, domainRepo v1DomainLookup) (*models.Domain, error) {
	return v1ResolveDomainForScope(c, domainRepo, models.DomainAPIScopeRead)
}

func v1ResolveDomainForScope(c fiber.Ctx, domainRepo v1DomainLookup, requiredScope string) (*models.Domain, error) {
	...
	if domain.APISecretRevokedAt != nil {
		err := fmt.Errorf("invalid API credentials")
		v1LogAuthFailure(c, domain, "api_key_revoked", err)
		return nil, err
	}
	if !v1DomainAllowsIP(domain, c.IP()) {
		err := fmt.Errorf("API key is not allowed from this IP")
		v1LogAuthFailure(c, domain, "api_ip_not_allowed", err)
		return nil, err
	}
	if !v1DomainHasScope(domain, requiredScope) {
		err := fmt.Errorf("API key scope is not allowed")
		v1LogAuthFailure(c, domain, "api_scope_denied", err)
		return nil, err
	}
	return domain, nil
}
...
func v1ResolveSignedDomain(c fiber.Ctx, domainRepo v1DomainLookup) (*models.Domain, error) {
	return v1ResolveSignedDomainForScope(c, domainRepo, models.DomainAPIScopeRead)
}

func v1ResolveSignedDomainForScope(c fiber.Ctx, domainRepo v1DomainLookup, requiredScope string) (*models.Domain, error) {
	domain, err := v1ResolveDomainForScope(c, domainRepo, requiredScope)
	if err != nil {
		return nil, err
	}
	...
	if secretDomain.APISecretRevokedAt != nil {
		err := fmt.Errorf("invalid API credentials")
		v1LogAuthFailure(c, domain, "api_secret_revoked", err)
		return nil, err
	}
	...
	if !v1RequestReplayAccepted(domainRepo, domain, c, timestamp, signature) {
		err := v1ReplayError()
...
func v1DomainHasScope(domain *models.Domain, requiredScope string) bool {
	requiredScope = strings.TrimSpace(requiredScope)
	if requiredScope == "" || domain == nil {
		return true
	}
	rawScopes := strings.TrimSpace(domain.APIScopes)
	if rawScopes == "" {
		return true
	}
	for _, scope := range strings.FieldsFunc(rawScopes, func(r rune) bool {
		return r == ',' || r == ';' || r == ' ' || r == '\n' || r == '\t'
	}) {
		scope = strings.TrimSpace(scope)
		if scope == models.DomainAPIScopeAll || scope == requiredScope {
			return true
		}
	}
	return false
...
func v1DomainAllowsIP(domain *models.Domain, clientIP string) bool {
	if domain == nil || strings.TrimSpace(domain.APIIPAllowlist) == "" {
		return true
	}
	...
	for _, entry := range strings.FieldsFunc(domain.APIIPAllowlist, func(r rune) bool {
		return r == ',' || r == ';' || r == ' ' || r == '\n' || r == '\t'
	}) {
		...
		if allowedIP := net.ParseIP(entry); allowedIP != nil && allowedIP.Equal(parsedIP) {
			return true
		}
	}
	return false
...
func HandleV1WalletCreate(deps V1APIDeps) fiber.Handler {
	return func(c fiber.Ctx) error {
		domain, err := v1ResolveSignedDomainForScope(c, deps.DomainRepo, models.DomainAPIScopeWalletCreate)
		if err != nil {
			return v1Err(c, fiber.StatusUnauthorized, err.Error())
		}
...
// @Description Creates a hosted payment link under the authenticated API domain. An optional chain_id/symbol/token sets the default checkout asset; omit them to let the payer choose from all available assets. Set link_type=donation to create a donation link where the payer chooses the crypto amount at checkout; amount and currency are ignored for donation links.
// @Tags Payment
// @Accept json
// @Produce json
// @Security ApiKeyAuth
...
func HandleV1PaymentLinkCreate(deps V1APIDeps) fiber.Handler {
	return func(c fiber.Ctx) error {
		domain, err := v1ResolveSignedDomainForScope(c, deps.DomainRepo, models.DomainAPIScopePaymentCreate)
		if err != nil {
			return v1Err(c, fiber.StatusUnauthorized, err.Error())
		}
...
func v1PaymentStatusTable() []types.V1StatusTableItem {
	return []types.V1StatusTableItem{
		...
		{Status: models.PaymentStatusPartialPaid, Description: "Partial deposit received; aggregate-enabled sessions may remain payable until cumulative finality reaches a terminal outcome.", IsFinal: false},
	}
}
...
const v1CreateMetadataMaxLen = 180

func v1CreateIdempotencyKey(c fiber.Ctx) string {
	return strings.TrimSpace(c.Get("Idempotency-Key"))
}

func validateV1CreateMetadata(idempotencyKey string, correlationID string) error {
	if len(idempotencyKey) > v1CreateMetadataMaxLen {
		return fmt.Errorf("Idempotency-Key must be at most %d characters", v1CreateMetadataMaxLen)
	}
	if len(correlationID) > v1CreateMetadataMaxLen {
		return fmt.Errorf("X-Request-ID must be at most %d characters", v1CreateMetadataMaxLen)
	}
	return nil
}
...
func completeV1CreateIdempotencyWithCompleteResource(ctx context.Context, repo *repositories.IdempotencyRepo, record *models.IdempotencyKey, resourceType string, resourceID uuid.UUID, response any) error {
	...
	if record.ID != uuid.Nil {
		if err := repo.CompleteResource(ctx, record.ID, resourceType, resourceID, body); err != nil {
			return err
		}
	} else {
		if err := repo.CompleteResourceByKey(ctx, record.DomainID, record.Key, resourceType, resourceID, body); err != nil {
			return err
		}
	}
	...
	record.ResourceID = &resourceIDCopy
...
func failV1CreateIdempotency(ctx context.Context, repo *repositories.IdempotencyRepo, record *models.IdempotencyKey, err error) {
	...
	if failErr := repo.Fail(ctx, record.ID, err.Error()); failErr != nil {
		log.Printf("v1 idempotency fail record_id=%s error=%v", record.ID, failErr)
	}
}

func enqueueV1PayoutRequestedLifecycle(ctx context.Context, deps V1APIDeps, domain models.Domain, request models.WithdrawalRequest) error {
...
func enqueueV1RefundRequestedLifecycle(ctx context.Context, deps V1APIDeps, domain models.Domain, refund models.Refund) error {
	if deps.WebhookDeliveryRepo == nil {
		return nil
	}
...
func openV1OutboundLifecycleReconciliation(ctx context.Context, deps V1APIDeps, chain string, merchantID uuid.UUID, domainID *uuid.UUID, resourceType string, resourceID string, lifecycleStatus string, reason string, err error, txHash string) {
	...
	_, _, openErr := deps.ReconciliationRepo.OpenStuckLifecycleJob(ctx, chainSlugToID(chain), &merchantID, domainID, resourceType, resourceID, lifecycleStatus, reason, evidence)
	if openErr != nil {
		log.Printf("v1 outbound lifecycle reconciliation open error: %v\n", openErr)
	}
}

func repairV1PayoutRequestedLifecycleOnReplay(ctx context.Context, deps V1APIDeps, domain models.Domain, record *models.IdempotencyKey) *models.IdempotencyKey {
	if deps.WithdrawalRepo == nil || record == nil || (record.ResourceType != "" && record.ResourceType != "payout") {
		return record
	}
	var request *models.WithdrawalRequest
	var err error
	if record.ResourceID != nil {
		request, err = deps.WithdrawalRepo.FindByDomain(ctx, domain.MerchantID, domain.ID, *record.ResourceID)
	} else {
		request, err = deps.WithdrawalRepo.FindByDomainIdempotencyKey(ctx, domain.MerchantID, domain.ID, record.Key)
	}
	if err != nil || request == nil {
		return record
	}
	if strings.TrimSpace(record.ResponseBody) == "" {
		response := fiber.Map{"result": "ok", "data": v1PayoutResponse(*request)}
		if err := completeV1CreateIdempotencyWithCompleteResource(ctx, deps.IdempotencyRepo, record, "payout", request.ID, response); err != nil {
			log.Printf("payout idempotency replay repair error: %v\n", err)
		}
	}
	if err := enqueueV1PayoutRequestedLifecycle(ctx, deps, domain, *request); err != nil {
		log.Printf("payout lifecycle webhook replay enqueue error: %v\n", err)
		openV1OutboundLifecycleReconciliation(ctx, deps, request.Chain, request.MerchantID, request.DomainID, "payout", request.ID.String(), request.Status, "outbound_requested_event_repair_failed", err, request.TxHash)
	}
	return record
}

func repairV1RefundRequestedLifecycleOnReplay(ctx context.Context, deps V1APIDeps, domain models.Domain, record *models.IdempotencyKey) *models.IdempotencyKey {
	if deps.RefundRepo == nil || record == nil || (record.ResourceType != "" && record.ResourceType != "refund") {
		return record
	}
	var refund *models.Refund
	var err error
	if record.ResourceID != nil {
		refund, err = deps.RefundRepo.FindByDomain(ctx, domain.MerchantID, domain.ID, *record.ResourceID)
	} else {
		refund, err = deps.RefundRepo.FindByDomainIdempotencyKey(ctx, domain.MerchantID, domain.ID, record.Key)
	}
	...
	if strings.TrimSpace(record.ResponseBody) == "" {
		...
		if err := completeV1CreateIdempotencyWithCompleteResource(ctx, deps.IdempotencyRepo, record, "refund", refund.ID, response); err != nil {
			log.Printf("refund idempotency replay repair error: %v\n", err)
		}
	}
	if err := enqueueV1RefundRequestedLifecycle(ctx, deps, domain, *refund); err != nil {
		log.Printf("refund lifecycle webhook replay enqueue error: %v\n", err)
		domainID := refund.DomainID
		openV1OutboundLifecycleReconciliation(ctx, deps, refund.Chain, refund.MerchantID, &domainID, "refund", refund.ID.String(), refund.Status, "outbound_requested_event_repair_failed", err, refund.TxHash)
	}
	return record
}
...
// @Success 200 {object} types.V1PayoutCreateResponse "Idempotent replay response"
...
func HandleV1PayoutCreate(deps V1APIDeps) fiber.Handler {
	return func(c fiber.Ctx) error {
		domain, err := v1ResolveSignedDomainForScope(c, deps.DomainRepo, models.DomainAPIScopePayoutCreate)
		if err != nil {
			return v1Err(c, fiber.StatusUnauthorized, err.Error())
		}
		...
		if !shouldCreate {
			idempotencyRecord = repairV1PayoutRequestedLifecycleOnReplay(c.Context(), deps, *domain, idempotencyRecord)
		...
		if err := completeV1CreateIdempotencyWithCompleteResource(c.Context(), deps.IdempotencyRepo, idempotencyRecord, "payout", req.ID, response); err != nil {
			openV1OutboundLifecycleReconciliation(c.Context(), deps, req.Chain, req.MerchantID, req.DomainID, "payout", req.ID.String(), req.Status, "outbound_idempotency_completion_failed", err, req.TxHash)
			return v1Err(c, fiber.StatusInternalServerError, "idempotency completion failed")
		}
		if err := enqueueV1PayoutRequestedLifecycle(c.Context(), deps, *domain, *req); err != nil {
			log.Printf("payout lifecycle webhook enqueue error: %v\n", err)
			openV1OutboundLifecycleReconciliation(c.Context(), deps, req.Chain, req.MerchantID, req.DomainID, "payout", req.ID.String(), req.Status, "outbound_requested_event_enqueue_failed", err, req.TxHash)
			return v1Err(c, fiber.StatusInternalServerError, "payout lifecycle webhook enqueue failed")
		}
...
// @Success 200 {object} types.V1RefundCreateResponse "Idempotent replay response"
...
func HandleV1RefundCreate(deps V1APIDeps) fiber.Handler {
	return func(c fiber.Ctx) error {
		domain, err := v1ResolveSignedDomainForScope(c, deps.DomainRepo, models.DomainAPIScopeRefundCreate)
		if err != nil {
			return v1Err(c, fiber.StatusUnauthorized, err.Error())
		}
		var body struct {
		...
		if !shouldCreate {
			idempotencyRecord = repairV1RefundRequestedLifecycleOnReplay(c.Context(), deps, *domain, idempotencyRecord)
			return replayV1CreateIdempotency(c, idempotencyRecord)
		}
		...
		response := fiber.Map{"result": "ok", "data": v1RefundResponse(*refund)}
		if err := completeV1CreateIdempotencyWithCompleteResource(c.Context(), deps.IdempotencyRepo, idempotencyRecord, "refund", refund.ID, response); err != nil {
			domainID := refund.DomainID
			openV1OutboundLifecycleReconciliation(c.Context(), deps, refund.Chain, refund.MerchantID, &domainID, "refund", refund.ID.String(), refund.Status, "outbound_idempotency_completion_failed", err, refund.TxHash)
			return v1Err(c, fiber.StatusInternalServerError, "idempotency completion failed")
		}
		if err := enqueueV1RefundRequestedLifecycle(c.Context(), deps, *domain, *refund); err != nil {
			log.Printf("refund lifecycle webhook enqueue error: %v\n", err)
			domainID := refund.DomainID
			openV1OutboundLifecycleReconciliation(c.Context(), deps, refund.Chain, refund.MerchantID, &domainID, "refund", refund.ID.String(), refund.Status, "outbound_requested_event_enqueue_failed", err, refund.TxHash)
			return v1Err(c, fiber.StatusInternalServerError, "refund lifecycle webhook enqueue failed")
		}
		return c.Status(fiber.StatusCreated).JSON(response)
	}
}
...
func v1RefundLimitRaw(ctx context.Context, deps V1APIDeps, session *models.PaymentSession) (*big.Int, error) {
	var limit *big.Int
	if models.IsDonationLinkType(session.LinkType) {
		if matched, ok := stringsToPositiveBigInt(session.MatchedAmountRaw); ok {
			limit = matched
		}
	}
	if limit == nil {
		expected, ok := stringsToPositiveBigInt(session.ExpectedAmountRaw)
		if !ok && !models.IsDonationLinkType(session.LinkType) {
	...
	if deps.TransactionRepo == nil || session.TxUniqueHash == nil || strings.TrimSpace(*session.TxUniqueHash) == "" {
		...
		return new(big.Int).Set(limit), nil
	}
	...
	if ok && (limit == nil || actual.Cmp(limit) > 0) {
		limit = actual
	}
	if limit == nil {
		return nil, fmt.Errorf("payment expected amount is invalid")
...
func v1PaymentLinkProductFromRequest(domain *models.Domain, body types.V1PaymentLinkCreateRequest) (*models.Product, error) {
	...
	token := strings.TrimSpace(body.Token)
	if body.ChainID == nil && (symbol != "" || token != "") {
		return nil, fmt.Errorf("chain_id is required when selecting a default asset")
	}
	if body.ChainID != nil && symbol == "" {
		return nil, fmt.Errorf("symbol is required when selecting a default asset")
	}
	if body.ChainID != nil && *body.ChainID < 0 {
	...
	if models.IsDonationLinkType(linkType) {
		if body.X402Enabled {
			return nil, fmt.Errorf("x402 is only supported for fixed-amount payment links")
		}
		amount = "0"
		currency = ""
	} else {
	...
	return &models.Product{
		...
		CancelURL:      cancelURL,
		LogoURL:        logoURL,
		X402Enabled:    body.X402Enabled,
		DefaultChainID: body.ChainID,
		DefaultSymbol:  symbol,
		DefaultToken:   optionalStringPtr(token),
		IsActive:       true,
	}, nil
}

func normalizePaymentLinkDefaultAsset(registry *asset.Registry, product *models.Product) error {
	if product == nil || product.DefaultChainID == nil {
		return nil
	}
	if registry == nil {
		return fmt.Errorf("asset registry is not configured")
	}
	token := ""
	if product.DefaultToken != nil {
		token = strings.TrimSpace(*product.DefaultToken)
	}
	assetInfo, err := findCheckoutAsset(registry, constants.ChainID(*product.DefaultChainID), product.DefaultSymbol, token)
	if err != nil {
		return err
	}
	chainID := int64(assetInfo.GetChainID())
	product.DefaultChainID = &chainID
	product.DefaultSymbol = strings.ToUpper(strings.TrimSpace(assetInfo.GetSymbol()))
	if assetInfo.IsNative() {
		product.DefaultToken = nil
		return nil
	}
	product.DefaultToken = optionalStringPtr(assetInfo.GetIdentifier())
	return nil
}
...
func v1PaymentLinkResponse(c fiber.Ctx, product models.Product) fiber.Map {
	response := fiber.Map{
		...
		"chain_id":         product.DefaultChainID,
	...
	if product.DefaultToken != nil {
		response["default_token"] = *product.DefaultToken
		response["token"] = *product.DefaultToken
	}
	if product.DefaultChainID != nil {
		defaultAsset := fiber.Map{
			"chain_id": *product.DefaultChainID,
			"symbol":   product.DefaultSymbol,
		}
		if product.DefaultToken != nil {
			defaultAsset["token"] = *product.DefaultToken
		}
		response["default_asset"] = defaultAsset
	}
	return response
}
...
func v1PaymentResponse(s models.PaymentSession) fiber.Map {
	...
	if strings.TrimSpace(string(s.ProductSnapshot)) != "" {
		response["product"] = s.ProductSnapshot
	}
	return response
}
