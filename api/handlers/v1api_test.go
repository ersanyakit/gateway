...
func TestNormalizePaymentLinkDefaultAssetResolvesTRONUSDT(t *testing.T) {
	...
	product := &models.Product{DefaultChainID: &chainID, DefaultSymbol: "usdt", DefaultToken: &token}

	if err := normalizePaymentLinkDefaultAsset(registry, product); err != nil {
		t.Fatal(err)
	}
	if product.DefaultChainID == nil || *product.DefaultChainID != int64(constants.TRON) || product.DefaultSymbol != "USDT" {
		t.Fatalf("normalized default asset = %#v", product)
	}
	if product.DefaultToken == nil || *product.DefaultToken != "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t" {
		t.Fatalf("normalized token = %v", product.DefaultToken)
	}
}

func TestPaymentLinkWithoutDefaultAssetLeavesAllAssetsAvailable(t *testing.T) {
	product, err := v1PaymentLinkProductFromRequest(&models.Domain{ID: uuid.New(), MerchantID: uuid.New()}, types.V1PaymentLinkCreateRequest{
		Name:     "All assets",
		Amount:   "10.00",
		Currency: "USD",
	})
	...
	if product.DefaultChainID != nil || product.DefaultSymbol != "" || product.DefaultToken != nil {
		t.Fatalf("default asset should be empty: %#v", product)
	}
}
...
func TestV1MutationHandlersRequireMatchingWriteScopes(t *testing.T) {
	source := readHandlerSource(t, "v1api.go")
	for _, tc := range []struct {
		function string
		scope    string
	}{
		{"HandleV1WalletCreate", "models.DomainAPIScopeWalletCreate"},
		{"HandleV1PaymentLinkCreate", "models.DomainAPIScopePaymentCreate"},
		{"HandleV1PaymentStaticAddressCreate", "models.DomainAPIScopeWalletCreate"},
		{"HandleV1PayoutCreate", "models.DomainAPIScopePayoutCreate"},
		{"HandleV1RefundCreate", "models.DomainAPIScopeRefundCreate"},
	} {
		body := extractHandlerFunctionBody(t, source, tc.function)
		want := "v1ResolveSignedDomainForScope(c, deps.DomainRepo, " + tc.scope + ")"
		if !strings.Contains(body, want) {
			t.Fatalf("%s must require %s; body:\n%s", tc.function, tc.scope, body)
		}
	}
}
