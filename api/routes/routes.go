...
func NewRouter(db *gorm.DB) *Router {
	...
	r.fiber.Use(middleware.TraceContext("gateway"))
	...
	r.fiber.Use("/api/v1", middleware.RateLimitAPIKeyWithStore(repositories.NewAPIRateLimitRepo(r.db)))

	x402Config, x402ConfigErr := x402svc.LoadConfigFromEnv()
	if x402ConfigErr != nil {
		log.Printf("[x402] seller middleware disabled: %v", x402ConfigErr)
	} else if x402Config.Enabled && len(x402Config.Routes) > 0 {
		r.fiber.Use(x402svc.NewMiddleware(x402Config))
		log.Printf("[x402] seller middleware enabled routes=%d networks=%v", len(x402Config.Routes), x402Config.Networks)
	}
	...
	r.MoneyEventInboxRepo = repositories.NewMoneyEventInboxRepo(r.db)
	r.WorkerLeaseRepo = repositories.NewWorkerLeaseRepo(r.db)
	r.WebhookDeliveryRepo = repositories.NewWebhookDeliveryRepo(r.db)
	r.SweepJobRepo = repositories.NewSweepJobRepo(r.db)
	r.OutboundTransactionRepo = repositories.NewOutboundTransactionRepo(r.db)
	...
	r.ProviderHealthRepo = repositories.NewProviderHealthRepo(r.db)
	r.WalletAddressLookupRepo = repositories.NewWalletAddressLookupRepo(r.db)
	...
	legacyCommandAccess := middleware.RequireLegacyCommandAccess()
	r.fiber.Post(constants.CMD_MERCHANT_CREATE.String(), legacyCommandAccess, handlers.HandleMerchantCreate(r.MerchantService))
	r.fiber.Post(constants.CMD_MERCHANT_FETCH.String(), legacyCommandAccess, handlers.HandleMerchantFetch(r.MerchantService))

	r.fiber.Post(constants.CMD_MERCHANT_DOMAIN_CREATE.String(), legacyCommandAccess, handlers.HandleDomainCreate(r.DomainService))
	r.fiber.Post(constants.CMD_DOMAIN_DEPOSIT_SUMMARY.String(), legacyCommandAccess, handlers.HandleDomainDepositSummary(r.TransactionRepo))

	r.fiber.Post(constants.CMD_MERCHANT_FETCH_BY_ID.String(), legacyCommandAccess, handlers.HandleMerchantFindById(r.MerchantService))
	r.fiber.Post(constants.CMD_MERCHANT_FETCH_BY_EMAIL.String(), legacyCommandAccess, handlers.HandleMerchantFindByEmail(r.MerchantService))

	r.fiber.Post(constants.CMD_MERCHANT_DELETE_BY_ID.String(), legacyCommandAccess, handlers.HandleMerchantDeleteById(r.MerchantService))
	r.fiber.Post(constants.CMD_MERCHANT_DELETE_BY_EMAIL.String(), legacyCommandAccess, handlers.HandleMerchantDeleteByEmail(r.MerchantService))

	r.fiber.Post(constants.CMD_MERCHANT_WALLET_CREATE.String(), legacyCommandAccess, handlers.HandleWalletCreate(r.WalletService))
	r.fiber.Post(constants.CMD_WITHDRAW.String(), legacyCommandAccess, handlers.HandleWithdraw(r.WalletRepo, r.blockchains))
	r.fiber.Post(constants.CMD_SWEEP.String(), legacyCommandAccess, handlers.HandleSweep(r.WalletRepo, r.blockchains))
	...
	r.fiber.Post("/admin/test-links", handlers.HandleAdminTestPaymentLinkCreate(dealerDeps))
	r.fiber.Post("/admin/test-deposit", handlers.HandleAdminTestDeposit(dealerDeps))
	r.fiber.Get("/admin/merchants/:id", handlers.HandleAdminMerchantDetail(dealerDeps))
	...
	r.fiber.Get("/metrics", handlers.HandleOperationalMetrics(handlers.OperationalMetricsDeps{
		...
		WorkerLeaseRepo:         r.WorkerLeaseRepo,
		SweepJobRepo:            r.SweepJobRepo,
		OutboundTransactionRepo: r.OutboundTransactionRepo,
		WithdrawalRepo:          r.WithdrawalRepo,
		RefundRepo:              r.RefundRepo,
		ReconciliationRepo:      r.ReconciliationRepo,
		ChainStateRepo:          r.ChainStateRepo,
...
func checkoutPostOnly(c fiber.Ctx) error {
...
func (r *Router) handlePacket(c fiber.Ctx) error {
	...
	c.Set("Access-Control-Allow-Headers", "Accept,Authorization,Content-Type,PAYMENT-SIGNATURE,X-Portal-JWT,Token,session,Origin,Host,Connection,Accept-Encoding,Accept-Language,X-Requested-With")
...
func resolveX402CheckoutPayment(ctx context.Context, repo *repositories.PaymentRepo, networkStates *repositories.NetworkOperationalStateRepo, token string) (x402svc.CheckoutPayment, error) {
	...
	if err != nil {
		...
		return x402svc.CheckoutPayment{}, err
	}
	if session == nil || !session.X402Enabled || session.Status != models.PaymentStatusAwaitingPayment {
		return x402svc.CheckoutPayment{}, x402svc.ErrCheckoutNotEligible
	}
	...
	return x402svc.CheckoutPayment{
		Network: network,
		PayTo:   session.DepositAddress,
		Asset:   strings.TrimSpace(*session.SelectedToken),
		Amount:  strings.TrimSpace(session.ExpectedAmountRaw),
	}, nil
}
...
func (r *Router) ProviderHealthService() *providerhealth.Service {
	return providerhealth.New(r.blockchains, r.ProviderHealthRepo, providerhealth.LoadConfigFromEnv())
}
