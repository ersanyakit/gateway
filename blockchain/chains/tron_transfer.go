...
func tronGetBlockRef(ctx context.Context, apiBase string) (*tronBlockRef, error) {
	...
	if blockNum < 0 {
		return nil, fmt.Errorf("tron block number must not be negative: %d", blockNum)
	}
	blockHashBytes, err := hex.DecodeString(result.BlockID)
	if err != nil {
		return nil, fmt.Errorf("tron blockID decode: %w", err)
	}
	if len(blockHashBytes) < 16 {
		return nil, fmt.Errorf("tron blockID is too short: got %d bytes", len(blockHashBytes))
	}
...
func tronGetTRC20Balance(ctx context.Context, rpcURL, contractAddr, ownerAddr string) (*big.Int, error) {
	...
	ownerHash, err := addrutil.TronAddressHash(ownerAddr)
	if err != nil {
		return nil, fmt.Errorf("tron address hash: %w", err)
	}
	// Pad to 32 bytes
	...
	contractHash, err := addrutil.TronAddressHash(contractAddr)
	if err != nil {
...
func tronPrivateKey(wallet blockchain.WalletDetails) (*ecdsa.PrivateKey, error) {
	...
	if wallet.Address != "" {
		derivedAddress, err := addrutil.TronAddressFromUncompressedPublicKey(crypto.FromECDSAPub(&privKey.PublicKey))
		if err != nil {
...
func tronSignRawTransaction(rawTxHex string, privKey *ecdsa.PrivateKey) (string, error) {
	dataToSign, err := tronSDK.SignStart(rawTxHex)
	if err != nil {
		return "", fmt.Errorf("tron sign start: %w", err)
	}
	hash, err := hex.DecodeString(dataToSign)
	if err != nil {
		return "", fmt.Errorf("tron sign hash decode: %w", err)
	}
	signature, err := crypto.Sign(hash, privKey)
	if err != nil {
		return "", fmt.Errorf("tron sign: %w", err)
	}
	compact := make([]byte, 0, len(signature))
	compact = append(compact, signature[64]+27)
	compact = append(compact, signature[:64]...)
	signedTx, err := tronSDK.SignEnd(rawTxHex, hex.EncodeToString(compact))
	if err != nil {
...
func tronSignAndBroadcast(ctx context.Context, apiBase, rawTxHex string, privKey *ecdsa.PrivateKey) (string, error) {
...
func tronSignBroadcastWithLease(ctx context.Context, apiBase, rawTxHex string, privKey *ecdsa.PrivateKey, lease *chainresource.SequenceLease) (string, error) {
	signedTx, err := tronSignRawTransaction(rawTxHex, privKey)
	if err != nil {
		_ = lease.Release()
		return "", err
...
func (s *TronChain) signTronRawWithCustody(ctx context.Context, wallet blockchain.WalletDetails, intent string, amountRaw string, destination string, rawTxHex string, payload map[string]any) (string, string, error) {
	if payload == nil {
		payload = make(map[string]any)
	}
	payload["format"] = "tron_raw_transaction.v1"
	payload["source_address"] = strings.TrimSpace(wallet.Address)
	payload["raw_transaction_hex"] = strings.TrimSpace(rawTxHex)
	response, err := signTransactionWithCustody(ctx, s.Name(), s.ChainID(), wallet, intent, amountRaw, destination, payload)
	if err != nil {
		return "", "", err
	}
	signedTxHex, err := signedPayloadHex(response)
	if err != nil {
		return "", "", fmt.Errorf("tron external signer transaction missing: %w", err)
	}
	return signedTxHex, strings.TrimSpace(response.TxHash), nil
}
...
func (s *TronChain) sendTRX(ctx context.Context, wallet blockchain.WalletDetails, amountRaw string, toAddress string) (*blockchain.TransactionResult, error) {
	...
	if err := requireDatabaseResourceReservation(ctx, s.Name(), "transfer.native"); err != nil {
		return nil, err
	}
	...
	if shouldUseExternalTransactionSigner(wallet) {
		signedTx, _, signErr := s.signTronRawWithCustody(ctx, wallet, "transfer.native", amount.String(), toAddress, rawTx, map[string]any{
			"to_address": toAddress,
			"amount_sun": sendAmount,
		})
		if signErr != nil {
			_ = lease.Release()
			return nil, signErr
		}
		txID, broadcastErr = tronBroadcastSignedWithLease(ctx, apiBase, signedTx, lease)
	} else {
		privKey, keyErr := tronPrivateKey(wallet)
		if keyErr != nil {
			_ = lease.Release()
			return nil, keyErr
		}
		txID, broadcastErr = tronSignBroadcastWithLease(ctx, apiBase, rawTx, privKey, lease)
	}
	if broadcastErr != nil {
...
func (s *TronChain) sendTRC20(ctx context.Context, wallet blockchain.WalletDetails, contractAddr, amountRaw, toAddress string) (*blockchain.TransactionResult, error) {
	if err := requireDatabaseResourceReservation(ctx, s.Name(), "transfer.token"); err != nil {
		return nil, err
	}
	if !s.ValidateAddress(wallet.Address) {
		return nil, fmt.Errorf("invalid tron wallet address: %s", wallet.Address)
	...
	var broadcastErr error
	if shouldUseExternalTransactionSigner(wallet) {
		signedTx, _, signErr := s.signTronRawWithCustody(ctx, wallet, "transfer.token", amount.String(), toAddress, rawTx, map[string]any{
			"contract_address": contractAddr,
			"to_address":       toAddress,
			"amount_raw":       amount.String(),
			"fee_limit_sun":    trc20FeeLimit,
		})
		if signErr != nil {
			_ = lease.Release()
			return nil, signErr
...
func (s *TronChain) SweepTo(ctx context.Context, wallet blockchain.WalletDetails, toAddress string) (*blockchain.TransactionResult, error) {
	if err := requireDatabaseResourceReservation(ctx, s.Name(), "sweep.native"); err != nil {
		return nil, err
	}
	if !s.ValidateAddress(wallet.Address) {
		return nil, fmt.Errorf("invalid tron wallet address: %s", wallet.Address)
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		if shouldUseExternalTransactionSigner(wallet) {
			signedTx, _, err = s.signTronRawWithCustody(ctx, wallet, "sweep.native", "max", toAddress, rawTx, map[string]any{
				"to_address":   toAddress,
				"amount_sun":   sendAmount,
				"fee_sun":      feeSUN,
				"sweep_native": true,
...
func (s *TronChain) SweepERC20To(ctx context.Context, wallet blockchain.WalletDetails, contractAddr, toAddress string) (*blockchain.TransactionResult, error) {
	if err := requireDatabaseResourceReservation(ctx, s.Name(), "sweep.token"); err != nil {
		return nil, err
	}
	if !s.ValidateAddress(wallet.Address) {
		return nil, fmt.Errorf("invalid tron wallet address: %s", wallet.Address)
	...
	var broadcastErr error
	if shouldUseExternalTransactionSigner(wallet) {
		signedTx, _, signErr := s.signTronRawWithCustody(ctx, wallet, "sweep.token", "max", toAddress, rawTx, map[string]any{
			"contract_address": contractAddr,
			"to_address":       toAddress,
			"amount_raw":       balance.String(),
			"fee_limit_sun":    trc20FeeLimit,
			"sweep_token":      true,
		})
		if signErr != nil {
...
func (s *TronChain) PrefundGas(ctx context.Context, reserveWallet blockchain.WalletDetails, userAddress string) (bool, error) {
	if err := requireDatabaseResourceReservation(ctx, s.Name(), "prefund.native"); err != nil {
		return false, err
	}
	...
	var broadcastErr error
	if shouldUseExternalTransactionSigner(reserveWallet) {
		signedTx, _, signErr := s.signTronRawWithCustody(ctx, reserveWallet, "prefund.native", fmt.Sprintf("%d", prefundAmountSUN), userAddress, rawTx, map[string]any{
			"to_address": userAddress,
			"amount_sun": prefundAmountSUN,
			"prefund":    true,
		})
		if signErr != nil {
			_ = lease.Release()
		...
		_, broadcastErr = tronBroadcastSignedWithLease(ctx, apiBase, signedTx, lease)
	} else {
		privKey, keyErr := tronPrivateKey(reserveWallet)
		if keyErr != nil {
			_ = lease.Release()
			return false, keyErr
		}
		_, broadcastErr = tronSignBroadcastWithLease(ctx, apiBase, rawTx, privKey, lease)
	}
	if broadcastErr != nil {
		return false, fmt.Errorf("tron prefund broadcast: %w", broadcastErr)
	}
	return true, nil
}
