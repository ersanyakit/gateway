...
9. Database migration strategy is not production-controlled.
   Startup `AutoMigrate` is useful for development, but production needs versioned migrations, rollback strategy, locks, and explicit DDL review.

10. Observability is not sufficient.
    The code still relies heavily on `log.Printf` / `fmt.Println` style output. There are no consistent structured logs, metrics, traces, backlog lag gauges, webhook SLOs, signer metrics, or alerting hooks.
