...
| `delivery_id` | Unique delivery row id. Replay attempts get their own delivery id. |
...
| `sequence` | Monotonic per-domain/per-resource delivery sequence. Consumers should process lower sequence values first for the same `resource_type` + `resource_id`. |
...
HTTP deliveries also include `X-Gateway-Delivery-Id`, `X-Gateway-Resource-Type`, `X-Gateway-Resource-Id`, `X-Gateway-Sequence`, and `X-Gateway-Idempotency-Key` when known. The signed body remains the canonical source of truth.

Delivery ordering is enforced per domain/resource lane. A later active delivery for the same resource is not claimed while an earlier pending, failed, or processing delivery remains active. Delivery remains at-least-once; consumers must deduplicate using `event_id` or `idempotency_key` and may use `delivery_id` to audit individual attempts/replays.
...
| `payment.partial_paid.v1` | payment | payment service | merchant, exchange, operator diagnostics | policy-dependent | Partial deposit received; aggregate-enabled sessions may remain payable until cumulative finality reaches a terminal outcome. |
...
Payment events add checkout context: `payment_id`, `order_id`, amount/currency fields, optional transaction hash, and failure, expiry, aggregate, memo/tag, or mismatch outcome reason when applicable. Payment-link sessions also include the immutable original product definition under `product` (including an optional `default_asset`); this is a session snapshot rather than a live product lookup. Explicit outcomes include `payment_outcome`, `matched_amount_raw`, `shortfall_amount_raw`, and `excess_amount_raw` where relevant. Reorg-corrected `payment.failed.v1` payloads also include `original_event_id`, `original_resource_id`, and `correction_reason`.
