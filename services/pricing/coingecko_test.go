...
func TestCoinGeckoPriceReturnsStaleCacheAndCoolsDownOnRateLimit(t *testing.T) {
	clearPriceFallbacks(t, "ETH")
	var requests int
	client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		requests++
		return &http.Response{
			...
			Body:       ioNopCloser(`{"error":"rate limited"}`),
	...
	if price.Cmp(stale) != 0 {
		t.Fatalf("cooldown price = %s, want stale %s", price, stale)
	}
	if requests != 1 {
		t.Fatalf("requests = %d, want cooldown to suppress second HTTP call", requests)
	}
}

func TestCoinGeckoPriceDoesNotHammerHTTPDuringRateLimitCooldown(t *testing.T) {
	clearPriceFallbacks(t, "ETH")
	var requests int
	client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		requests++
	...
	if _, err := oracle.Price(context.Background(), "ETH", "USD"); err == nil {
		t.Fatal("first rate-limited request should fail without stale cache")
	}
	if _, err := oracle.Price(context.Background(), "ETH", "USD"); err == nil || !strings.Contains(err.Error(), "rate limit cooldown active") {
		t.Fatalf("second request should fail from local cooldown, got %v", err)
	}
	if requests != 1 {
		t.Fatalf("requests = %d, want cooldown to suppress second HTTP call", requests)
	}
