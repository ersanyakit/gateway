...
func reservationFinalizationError(kind, action string, err error) error {
	log.Printf("chain resource kind=%s action=%s error=%v", kind, action, err)
	return err
}
...
func (r *NonceReservation) Release() error {
	if r == nil || r.manager == nil {
		return reservationFinalizationError("nonce", "release", fmt.Errorf("%w: nil nonce reservation", ErrInvalidResourceRequest))
	}
	r.manager.mu.Lock()
	defer r.manager.mu.Unlock()
	if r.done {
		return reservationFinalizationError("nonce", "release", ErrReservationFinalized)
	}
...
func (r *NonceReservation) Consume(txHash string) error {
	if r == nil || r.manager == nil {
		return reservationFinalizationError("nonce", "consume", fmt.Errorf("%w: nil nonce reservation", ErrInvalidResourceRequest))
	}
	r.manager.mu.Lock()
	defer r.manager.mu.Unlock()
	if r.done {
		return reservationFinalizationError("nonce", "consume", ErrReservationFinalized)
	}
	delete(r.manager.activeNonces, r.key)
...
func (r *UTXOReservation) Release() error {
	if r == nil || r.manager == nil {
		return reservationFinalizationError("utxo", "release", fmt.Errorf("%w: nil utxo reservation", ErrInvalidResourceRequest))
	}
	r.manager.mu.Lock()
	defer r.manager.mu.Unlock()
	if r.done {
		return reservationFinalizationError("utxo", "release", ErrReservationFinalized)
	}
...
func (r *UTXOReservation) Consume(txHash string) error {
	if r == nil || r.manager == nil {
		return reservationFinalizationError("utxo", "consume", fmt.Errorf("%w: nil utxo reservation", ErrInvalidResourceRequest))
	}
	r.manager.mu.Lock()
	defer r.manager.mu.Unlock()
	if r.done {
		return reservationFinalizationError("utxo", "consume", ErrReservationFinalized)
	}
...
func (r *SequenceLease) Release() error {
	if r == nil || r.manager == nil {
		return reservationFinalizationError("sequence", "release", fmt.Errorf("%w: nil sequence lease", ErrInvalidResourceRequest))
	}
	r.manager.mu.Lock()
	defer r.manager.mu.Unlock()
	if r.done {
		return reservationFinalizationError("sequence", "release", ErrReservationFinalized)
	}
...
func (r *SequenceLease) Consume(txHash string) error {
	if r == nil || r.manager == nil {
		return reservationFinalizationError("sequence", "consume", fmt.Errorf("%w: nil sequence lease", ErrInvalidResourceRequest))
	}
	r.manager.mu.Lock()
	defer r.manager.mu.Unlock()
	if r.done {
		return reservationFinalizationError("sequence", "consume", ErrReservationFinalized)
	}
	delete(r.manager.activeSequences, r.key)
	r.done = true
	return nil
