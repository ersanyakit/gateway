...
func Authorize(ctx context.Context, req Request) (Decision, error) {
	...
	case productionLocalSecretBoundary(req):
		decision.PolicyDecision = PolicyDecisionDeny
		decision.Outcome = OutcomeRejected
		decision.Reason = "production_secret_material_access_disabled"
		err = ErrProductionSecretMaterialAccessDisabled
	case IsExternalMode(mode):
		if ok, _, adapterErr := externalAdapterReady(ctx, mode); !ok {
			decision.PolicyDecision = PolicyDecisionDeny
			decision.Outcome = OutcomeRejected
...
func ProductionReadiness() (bool, string, error) {
	...
	if err := ensureConfiguredCustodyAdapterFromEnv(); err != nil {
		return false, "configured custody signer adapter is invalid", err
	}
	...
	case IsExternalMode(mode):
		if ok, provider, err := externalAdapterReady(context.Background(), mode); ok {
			details := fmt.Sprintf("SIGNER_MODE=%s external signer adapter is active", mode)
			if provider != "" {
				details += " provider=" + provider
			}
			return true, details, nil
		} else {
			return false, fmt.Sprintf("SIGNER_MODE=%s is declared but no external signer adapter is active", mode), err
		}
	default:
...
func IsExternalMode(mode string) bool {
	...
	case "kms", "hsm", "mpc", "vault", "external", "custody", "external_custody", "first_party", "first_party_custody", "internal_custody", "remote", "remote_signer", "custody_signer":
		return true
	default:
		return false
	}
...
func NormalizeMode(value string) string {
	...
	return strings.NewReplacer("-", "_", " ", "_").Replace(normalized)
}
...
func writeAudit(req Request, decision Decision) {
	...
	auditSink.Lock()
	_, err := auditSink.output.Write([]byte(b.String()))
	auditSink.Unlock()
	if err != nil {
		log.Printf("signer policy audit write error=%v", err)
	}
}
