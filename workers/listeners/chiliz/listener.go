...
func (r *RpcListener) Start() error {
	...
	helpers.GoSafely("listener.chiliz.read", r.readLoop)
	helpers.GoSafely("listener.chiliz.subscribe-transfers", r.subscribeTransfers)
	helpers.GoSafely("listener.chiliz.subscribe-heads", r.subscribeNewHeads)
...
func (r *RpcListener) subscribeTransfers() {
	...
	if err := r.writeJSON(req); err != nil {
		log.Printf("chiliz transfer subscription write error: %v", err)
	}
}
...
func (r *RpcListener) subscribeNewHeads() {
	...
	if err := r.writeJSON(req); err != nil {
		log.Printf("chiliz head subscription write error: %v", err)
	}
}
...
func (r *RpcListener) rpcCall(method string, params []interface{}) int {
	...
	if err := r.writeJSON(req); err != nil {
		log.Printf("chiliz rpc request method=%s id=%d write error: %v", method, id, err)
	}
	return id
}
	...
	for {
		...
		default:
			...
			if err := json.Unmarshal(msg, &rpcMsg); err != nil {
				log.Printf("chiliz websocket message decode error: %v\n", err)
				continue
			}
			...
			if rpcMsg.Method == "eth_subscription" {
				...
				if err := json.Unmarshal(rpcMsg.Params.Result, &header); err == nil && header.Number != "" {
					helpers.GoSafely("listener.chiliz.handle-block", func() { r.handleNewBlock(header.Number) })
					continue
				}
			}
	...
	for {
		...
		default:
			if err := r.connect(); err == nil {
				...
				helpers.GoSafely("listener.chiliz.resubscribe-transfers", r.subscribeTransfers)
				helpers.GoSafely("listener.chiliz.resubscribe-heads", r.subscribeNewHeads)
				return
			}
