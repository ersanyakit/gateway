...
func (r *RpcListener) Start() error {
	...
	helpers.GoSafely("listener.ethereum.read", r.readLoop)
	helpers.GoSafely("listener.ethereum.subscribe-transfers", r.subscribeTransfers)
	helpers.GoSafely("listener.ethereum.subscribe-heads", r.subscribeNewHeads)
...
func (r *RpcListener) subscribeTransfers() {
	...
	if err := r.writeJSON(req); err != nil {
		log.Printf("ethereum transfer subscription write error: %v", err)
	}
}
...
func (r *RpcListener) subscribeNewHeads() {
	...
	if err := r.writeJSON(req); err != nil {
		log.Printf("ethereum head subscription write error: %v", err)
	}
}
...
func (r *RpcListener) rpcCall(method string, params []interface{}) int {
	...
	if err := r.writeJSON(req); err != nil {
		log.Printf("ethereum rpc request method=%s id=%d write error: %v", method, id, err)
	}
	return id
}
	...
	for {
		...
		default:
			...
			if err := json.Unmarshal(msg, &rpcMsg); err != nil {
				log.Printf("ethereum websocket message decode error: %v\n", err)
				continue
			}
			...
			if rpcMsg.Method == "eth_subscription" {
				...
				if err := json.Unmarshal(rpcMsg.Params.Result, &header); err == nil && header.Number != "" {
					helpers.GoSafely("listener.ethereum.handle-block", func() { r.handleNewBlock(header.Number) })
					continue
				}
			}
	...
	for {
		...
		default:
			if err := r.connect(); err == nil {
				...
				helpers.GoSafely("listener.ethereum.resubscribe-transfers", r.subscribeTransfers)
				helpers.GoSafely("listener.ethereum.resubscribe-heads", r.subscribeNewHeads)
				return
			}
